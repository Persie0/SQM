...
*/
#include "mySSD1306AsciiUC.h"

//  18022017 Edited by R Brown to reduce memory foot print for mySQMGPSPRO
//  DO NOT REPLACE THIS FILE

//------------------------------------------------------------------------------

void SSD1306Ascii::Display_Normal()
{
  // Display_Normal();
  // White on Black
  ssd1306WriteCmd( 0xA6 );
}

void SSD1306Ascii::Display_Inverse()
{
  // Display_Inverse();
  // Black on white
  ssd1306WriteCmd( 0xA7 );
}

void SSD1306Ascii::Display_Off()
{
  ssd1306WriteCmd(0xAE);
}

void SSD1306Ascii::Display_On()
{
  ssd1306WriteCmd(0xAF);
}

void SSD1306Ascii::Display_Bright()
{
  // Display Bright
  setContrast(0xFF);
}

void SSD1306Ascii::Display_Dim()
{
  // Display Dim
  setContrast(0x00);
}

void SSD1306Ascii::Display_Rotate( int mode )
{
  if ( mode == 0x01 )
  {
    ssd1306WriteCmd( ((0xC8) >> 8) | 0xC0 );    // Portrait 180 degree, upside down
    ssd1306WriteCmd( ((0xA0) >> 8) | 0xA0 );
  }
  else if ( mode == 0x00 )
  {
    ssd1306WriteCmd(0xA0 | 0x01);
    ssd1306WriteCmd(0xC8);                      // Portrait Normal
  }
  else
  {
...
void SSD1306Ascii::InverseCharOn( void )
{
  m_invertChar = true;
}

void SSD1306Ascii::InverseCharOff( void )
{
  m_invertChar = false;
}
    ...
    for (uint8_t m = 0; m < m_magFactor; m++)
    {
      ...
      for (uint8_t c = 0; c < w; c++)
      {
        ...
        if (m_magFactor == 2)
        {
          ...
          if ( m_invertChar == false )
            ssd1306WriteRamBuf(b);
          else
            ssd1306WriteRamBuf(b ^ 0b01111111);
        }
        //------------------------------------------------------------------------------
        if ( m_invertChar == false )
          ssd1306WriteRamBuf(b);
